...
function showChartTooltip(target,e){let tip=$("#chartTooltip");if(!tip||!target)return;tip.innerHTML=chartTooltipHtml(target);tip.style.display="block";document.querySelectorAll(".chart-series.active").forEach(g=>g.classList.toggle("active",false));let series=target.closest(".chart-series");if(series)series.classList.toggle("active",true);moveChartTooltip(e)}
function hideChartTooltip(){let tip=$("#chartTooltip");if(tip)tip.style.display="none";document.querySelectorAll(".chart-series.active").forEach(g=>g.classList.toggle("active",false))}
...
$("#teamTrendChart").addEventListener("mousemove",e=>{let t=e.target.closest(".chart-hover-target");if(!t)return;showChartTooltip(t,e)});
$("#teamTrendChart").addEventListener("mouseleave",hideChartTooltip);
