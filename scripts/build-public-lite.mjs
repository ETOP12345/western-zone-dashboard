...
<main>
<section class="status-band"><div><span class="label">Current view</span><strong id="currentView">--</strong><small>Team Pacific Northwest LSC</small></div><div><span class="label">Swimmer filter</span><strong id="selectedSwimmer">All top 50</strong><small id="selectedSwimmerStatus">no individual selected</small></div><div><span class="label">Rank changes</span><strong id="rankChangeSummary">--</strong><small>compared with previous build</small></div><div><span class="label">Last loaded</span><strong id="lastUpdated">--</strong><small id="dataSource">--</small></div></section>
...
function render(){document.body.classList.toggle("mobile-detail-open",state.mobileDetail);refreshSwimmerSelect();let rows=filteredRows();$("#applicantCount").textContent=rows.length+" of top "+Math.min(LIMIT,currentAll().length)+" swimmers";$("#lastUpdated").textContent=fmtDateTime(DATA.lastLoadedAt||DATA.lastUpdated||DATA.generatedAt);$("#dataSource").textContent=(DATA.source||"--")+" · public";$("#currentView").textContent=state.ageGroup+" "+genderLabel();$("#rankChangeSummary").textContent=rankSummaryText();renderSelectedSummary();renderTable(rows);renderDetail()}
function renderSelectedSummary(){let r=state.selected?findSwimmer(state.selected):null;if(!r){$("#selectedSwimmer").textContent="All top 50";$("#selectedSwimmerStatus").textContent="select any swimmer for details";return}$("#selectedSwimmer").textContent="#"+r.rank+" "+r.name;$("#selectedSwimmerStatus").textContent=(r.score>=999?"Incomplete":"score "+r.score)+" · "+r.status+" · "+rankChangeText(r)}
...
function findSwimmer(name){return currentAll().find(x=>x.name===name)||currentRows()[0]}
