...
<nav class="tabs" aria-label="Dashboard views"><button id="swimmersTab" class="tab-button active" type="button" data-tab="swimmers">Swimmers</button><button id="teamsTab" class="tab-button" type="button" data-tab="teams">Team Distribution</button></nav>
...
const DATA=${json};
const LIMIT=50;
let state={ageGroup:DATA.defaultAgeGroup,gender:DATA.defaultGender,selected:"",detail:{type:"none"},mobileDetail:false,tab:"swimmers"};
...
function renderTeamTrendChart(){let teams=teamTrendRows();let ageGroups=DATA.ageGroups;let colors=["#1d4ed8","#157347","#b42318","#996515","#7c3aed","#0f766e","#c2410c","#be185d","#334155","#0891b2","#4d7c0f","#9333ea"];let w=920,h=320,l=54,r=24,t=24,b=48,iw=w-l-r,ih=h-t-b;let max=Math.max(1,...teams.flatMap(team=>team.counts));let x=i=>l+(ageGroups.length===1?iw/2:i*iw/(ageGroups.length-1));let y=v=>t+(max-v)*ih/max;let grid=[0,.25,.5,.75,1].map(p=>{let val=Math.round(max*p);let yy=y(val);return '<line class="chart-grid" x1="'+l+'" x2="'+(w-r)+'" y1="'+yy+'" y2="'+yy+'"></line><text class="chart-label" x="'+(l-10)+'" y="'+(yy+4)+'" text-anchor="end">'+val+'</text>'}).join("");let xLabels=ageGroups.map((g,i)=>'<text class="chart-label" x="'+x(i)+'" y="'+(h-16)+'" text-anchor="middle">'+esc(g)+'</text>').join("");let lines=teams.map((team,i)=>{let color=colors[i%colors.length];let points=team.counts.map((count,idx)=>x(idx)+","+y(count)).join(" ");let cls=i<12?"chart-team-line":"chart-team-line minor";let dots=i<12?team.counts.map((count,idx)=>'<circle class="chart-dot" cx="'+x(idx)+'" cy="'+y(count)+'" r="3.5" fill="'+color+'"><title>'+esc(team.team+" "+ageGroups[idx]+": "+count)+'</title></circle>').join(""):"";return '<polyline class="'+cls+'" points="'+points+'" stroke="'+color+'"><title>'+esc(team.team+": "+team.counts.join(", "))+'</title></polyline>'+dots}).join("");let legend=teams.map((team,i)=>'<span class="legend-item"><span class="legend-swatch" style="background:'+colors[i%colors.length]+'"></span>'+esc(team.team)+' ('+team.total+')</span>').join("");$("#teamTrendCount").textContent=teams.length+" teams · "+genderLabel()+" · top 50 per age group";$("#teamTrendChart").innerHTML=teams.length?'<div class="chart-wrap"><svg class="team-line-chart" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="Top 50 swimmer counts by team and age group"><rect x="0" y="0" width="'+w+'" height="'+h+'" fill="white"></rect>'+grid+'<line class="chart-axis" x1="'+l+'" x2="'+(w-r)+'" y1="'+(h-b)+'" y2="'+(h-b)+'"></line><line class="chart-axis" x1="'+l+'" x2="'+l+'" y1="'+t+'" y2="'+(h-b)+'"></line>'+lines+xLabels+'<text class="chart-label" x="'+(w/2)+'" y="'+(h-2)+'" text-anchor="middle">Age group</text><text class="chart-label" transform="translate(14 '+(h/2)+') rotate(-90)" text-anchor="middle">Top 50 swimmers</text></svg></div><div class="chart-legend">'+legend+'</div>':'<div class="empty">No team trend data loaded for this gender.</div>'}
