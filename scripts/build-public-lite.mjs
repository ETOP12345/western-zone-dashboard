...
const AAA_CUTS = {
  ...
  SCM_AA: { "50 Free": "28.29", "100 Free": "1:01.39", "200 Free": "2:12.99", "400 Free": "4:41.79", "50 Back": "32.69", "100 Back": "1:10.89", "200 Back": "2:31.49", "50 Breast": "37.19", "100 Breast": "1:21.39", "200 Breast": "2:55.99", "50 Fly": "31.19", "100 Fly": "1:10.39", "200 Fly": "2:38.99", "100 IM": "1:11.99", "200 IM": "2:31.19", "400 IM": "5:22.49" }
};
...
function countCuts(swimmer) {
  ...
  return {
    ...
    scmAA: eligible.filter(s => s.course === "SCM" && qualifies(s, AAA_CUTS.SCM_AA)).length
  };
}
...
function tieType(swim) {
  if (swim.course === "LCM" && qualifies(swim, AAA_CUTS.LCM)) return "LCM AAA";
  if (swim.course === "SCY" && qualifies(swim, AAA_CUTS.SCY)) return "SCY AAA";
  if (swim.course === "SCM" && qualifies(swim, AAA_CUTS.SCM_AA)) return "SCM AA";
  return "";
}
...
$("#ageGroupSelect").addEventListener("change",e=>{state.ageGroup=e.target.value;state.selected="";state.detail={type:"swimmer",name:TARGET};render()});
$("#genderSelect").addEventListener("change",e=>{state.gender=e.target.value;state.selected="";state.detail={type:"swimmer",name:TARGET};render()});
$("#swimmerSelect").addEventListener("change",e=>{state.selected=e.target.value;state.detail=e.target.value?{type:"swimmer",name:e.target.value}:{type:"swimmer",name:TARGET};render()});
$("#resetDetail").addEventListener("click",()=>{state.ageGroup="11-12";state.gender="M";state.selected=TARGET;state.detail={type:"swimmer",name:TARGET};$("#ageGroupSelect").value=state.ageGroup;$("#genderSelect").value=state.gender;render()});
$("#rankingBody").addEventListener("click",e=>{let t=e.target.closest("[data-name],[data-team],[data-event]");if(!t){let tr=e.target.closest("tr");if(tr)t=tr}if(!t)return;if(t.classList.contains("team-link"))state.detail={type:"team",team:t.dataset.team};else if(t.classList.contains("event-link"))state.detail={type:"event",event:t.dataset.event};else if(t.classList.contains("tie-link"))state.detail={type:"tie",name:t.dataset.name};else state.detail={type:"swimmer",name:t.dataset.name};render()});
